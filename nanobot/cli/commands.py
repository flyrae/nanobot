...
def gateway(
    ...
    provider_name = config.get_provider_name(config.agents.defaults.model) or "auto"
    provider_type = config.agents.defaults.provider
    ...
    enabled_ch = []
    for name in ["whatsapp", "telegram", "discord", "feishu", "dingtalk", "email", "slack", "qq", "ws_client", "wecom", "weixin"]:
        ch_cfg = getattr(config.channels, name, None)
        if ch_cfg and getattr(ch_cfg, "enabled", False):
            enabled_ch.append(name)
